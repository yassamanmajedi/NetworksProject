...
class GUI:
    ...
    def browse_file(self):
        ...
        self.file_name_widget.configure(text=self.filename)
        self.send_file_button.configure(state='active')

    def send_file(self):
        with open(self.filename, 'rb') as f:
            chunk = f.read(2048)
            self.client_socket.send(
                ('START_OF_FILE').encode('utf-8'))
            while chunk:
                self.client_socket.send(chunk)
                chunk = f.read(2048)
            self.client_socket.send(
                ('END_OF_FILE').encode('utf-8'))
            self.client_socket.shutdown(socket.SHUT_WR)
            f.close()

    def send_file_handle(self):
        thread = threading.Thread(target=self.send_file)
        thread.start()

    def display_send_file(self):
        frame = Frame()
        self.file_name_widget = Label(frame, width=50, borderwidth=2)
        self.file_name_widget.pack(side='left', anchor='e')
        self.explore_button = Button(frame,
                                     text="Browse Files",
                                     width=10,
                                     command=self.browse_file).pack(side='left')
        self.send_file_button = Button(frame,
                                       text="Send File",
                                       width=10,
                                       state='disabled',
                                       command=self.send_file_handle)
